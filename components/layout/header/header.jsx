		...
		<header className={styles['header'] + ' header'}>
						...
						<div className={styles['top-header'] + ' top-header'}>
							{topMenu[0].field_top_menu_links.map((item) => (
								<>
									<a
										key={`top-${item.uri}`}
										href={item.uri.replace('internal:', '')}
									>
										{item.title}
									</a>
								</>
			...
			<nav className={`d-lg-none d-block nav`}>
				<Container className={styles['container']}>
					<div className={styles['navbar-left']}>
						<a href="#">
							<Image src={Logo} width={166} height={58} alt="image page" />
						</a>
					...
					<div className={styles['main-menu'] + ' main-menu'}>
						<ul>
							...
							{mainMenu.map((item) => {
								return (
									<li key={`mobile-${item.id}`} className={styles.navItem}>
